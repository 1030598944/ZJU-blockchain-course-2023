...
const LotteryPage = () => {
    ...
    useEffect(() => {
        ...
        const getInfo = async () => {
            if (borrowYourCarContract) {
                ...
                const u = ownerCars.map((carId: number) => new Car(carId));
                setunborrowcar(u);
            } else {
    ...
    const borrowcar = async () => {
        ...
        //     try {
        //         // 获取一辆新车
        //         const owner =  await borrowYourCarContract.methods.getowner(inputborrowid).call()
        //         const borrower =  await borrowYourCarContract.methods.getwhoborrowcar(inputborrowid).call()
        //
        //         if(owner === '0x0000000000000000000000000000000000000000'){
        //
        //             alert('该车辆不存在,请检查输入的车辆ID是否正确')
